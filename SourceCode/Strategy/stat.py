...
class result:
    def __init__(self):
        self.res = {'wina'	:	0,  # self's win as offensive
                        'winb'		:	0,  # self's win as defensive
                        'losea'		:	0,  # self's lose as offensive
                        'loseb'		:	0,  # self's' lose as defensive
                        'tie'		:	0,  # tie
                        'bug'		:	0,  # self's bug
...
def main():	
    ...
    resfile.write("id\twina\twinb\tlosea\tloseb\ttie\tbug\tdebug\tillegal\tdeillegal\ttimeout\tdetimeout\tloaderr"
                  "\tdeloaderr\tnoentry\tdenoentry\n")
    for i in idList:
        res = idList[i].res
        line = str(i) + "\t" + str(res['wina']) + "\t" + str(res['winb']) + "\t" + str(res['losea']) + "\t" + str \
            (res['loseb']) + "\t" + str(res['tie']) + "\t" + str(res['bug']) + "\t" + str(res['debug']) + "\t" + str \
            (res['illegal']) + "\t" + str(res['deillegal']) + "\t" + str(res['timeout']) + "\t" + str \
            (res['detimeout']) + "\t" + str(res['loaderr']) + "\t" + str(res['deloaderr']) + "\t" + str \
            (res['noentry']) + "\t" + str(res['denoentry'])
        resfile.write(line + "\n")
    resfile.close()
